...
def sample_stats(sample) -> dict:
    ...
    engine_moves = {r["best_move"] for r in results.values()}
    legal = [p for p in (best, refutation) if p.outcome == Outcome.LEGAL]
    return {
        "calls": len(calls),
        "errors": sum(r.error is not None for r in replies.values()),
        "answered_after_tool": bool(results) and not sample.output.message.tool_calls,
        "queried_given": given is not None,
        "best_legal": best.outcome == Outcome.LEGAL,
        "relay": best.outcome == Outcome.LEGAL and given is not None and best.uci == given["best_move"],
        "legal_fields": len(legal),
        "beyond_engine": sum(p.uci not in engine_moves for p in legal),
        "frame_error": after is not None and as_reply.outcome == Outcome.LEGAL and as_reply.uci == after["best_move"],
    }
